...
- 按 arXiv 美东公告发布时间批次从 arXiv 抓取新论文，默认分类为 `cs.RO`、`cs.CV`、`cs.LG`、`cs.AI`、`eess.SY`。
...
`ARXIV_DAILY_NETWORK_FETCH_LIMIT` 用于限制同一 arXiv 公告批次每天“有效拉取”的次数；默认 `5`。只有成功完成且保存到新论文的官方 API 抓取才计数，429、503、超时、连接失败、缓存命中和 0 篇新增都不计数。普通抓取会优先使用本地缓存重算关键词。设置为 `0` 表示不限制。

最近两个 arXiv 公告批次的空结果缓存默认只复用 30 分钟，避免太早抓取到 `totalResults=0` 后挡住后续更新；可用 `ARXIV_EMPTY_CACHE_TTL_SECONDS` 调整，设置为 `0` 表示不过期。

日期选择对应 arXiv 的公告发布批次日，而不是北京时间自然日。系统默认展示最近已经发布的公告批次；下拉框和“前一批/后一批”导航只显示可抓取批次。周日批次覆盖上周四 14:00 到周五 14:00 前的提交；周一批次覆盖上周五 14:00 到周一 14:00 前的提交；周二到周四批次覆盖前一工作日 14:00 到当天 14:00 前的提交。周五和周六没有常规公告。
